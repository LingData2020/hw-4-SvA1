...
5. How many left- and right-handed participants are present?
...
```{r}
sprintf('Number of participants: %s', length(unique(mono_socio[["ParticipantID"]])))

gender <- mono_socio %>% filter(QuestionCategory=="sex")%>% count(Responce)
sprintf('%s males and %s females', gender$n[[2]], gender$n[[1]])

edu <- mono_socio %>% filter(QuestionCategory=="education") %>% distinct(Responce) 
levels <- paste( unlist(c(edu[,1])), collapse=', ')
sprintf('In df following education levels are presented: %s', levels)
...
sprintf(' %s left-randed and %s right-randed participants', hand$n[[1]], hand$n[[2]])
```
...
```
RT doesn't follow normal distribution because it's not symmetrically divided by the mean and it's got a long tail on the right
...
```
We can claim that it almost follows normal distribution, however, it's  still asymmetrical.
```
...
ggplot(mono1, aes(x = AcceptabilityJ, y = RTlog))+
  ...
  geom_boxplot(aes(color = WordOrder), width = 0.2, position = position_dodge(1))+
  labs()
...
```
p-value = 0.009152, it is less than 5%, so H0 should be rejected.
```

What claim about logarithms of reaction time for Congruent and Incongruent stimuli can you make now?
```
They are different, so H0 can be rejected. 
...
There's a difference between paired t-test result and 
two-sample t-test result because in two-sample t-test 
person-to-person variability has a strong impact. However, we don't really care about this variability . Paired t-test reduces this impact.
```
...
```{r}
result.aov <- aov(RTlog ~ TypeOfStimuli, data = incong_rtlog)
summary(result.aov)
```

```
p-value = 0.0125 < 5%, so H0 should be rejected. It can be claimed that one can find the difference in logarithm of reaction time for different conditions statistically significant.
...
```
```
p-value = 0.0151255 <5% (SubjectiveComment-Material-Shape-Color)
The difference is statistically significant between the logarithms of reaction time.
```
